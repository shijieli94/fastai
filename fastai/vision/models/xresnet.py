...
class XResNet(nn.Sequential):
    ...
    def __init__(
        ...
        **kwargs
        ...
        block_szs = [
            int(o * widen) for o in [64, 128, 256, 512] + [256] * (len(layers) - 4)
        ]
        ...
        super().__init__(
            ...
            *blocks,
            AdaptiveAvgPool(sz=1, ndim=ndim),
            Flatten(),
            nn.Dropout(p),
...
def _xresnet(pretrained, expansion, layers, **kwargs):
    ...
    if pretrained:
        res.load_state_dict(
            load_state_dict_from_url(url, map_location="cpu")["model"], strict=False
        )
    return res
...
def xse_resnext18_deeper(n_out=1000, pretrained=False, **kwargs):
    return XResNet(
        SEResNeXtBlock, 1, [2, 2, 1, 1, 1, 1, 1, 1], n_out=n_out, **se_kwargs2, **kwargs
    )


def xse_resnext34_deeper(n_out=1000, pretrained=False, **kwargs):
    return XResNet(
        SEResNeXtBlock, 1, [3, 4, 4, 2, 2, 1, 1, 1], n_out=n_out, **se_kwargs2, **kwargs
    )


def xse_resnext50_deeper(n_out=1000, pretrained=False, **kwargs):
    return XResNet(
        SEResNeXtBlock, 4, [3, 4, 4, 2, 2, 1, 1, 1], n_out=n_out, **se_kwargs2, **kwargs
    )
