...
class RandTransform(DisplayedTransform):
    ...
    def __call__(
        self, b, split_idx: int = None, **kwargs  # Index of the train/valid dataset
    ):
...
mk_class(
    "PadMode",
    **{o: o.lower() for o in ["Zeros", "Border", "Reflection"]},
...
@patch
def _do_crop_pad(
    x: TensorPoint, sz, tl, orig_sz, pad_mode=PadMode.Zeros, resize_to=None, **kwargs
):
    ...
    return TensorPoint(
        (x + 1) * orig_sz / sz - tl * 2 / sz - 1,
        sz=sz if resize_to is None else resize_to,
    )


@patch
def _do_crop_pad(
    x: TensorBBox, sz, tl, orig_sz, pad_mode=PadMode.Zeros, resize_to=None, **kwargs
):
    bbox = TensorPoint._do_crop_pad(
        x.view(-1, 2), sz, tl, orig_sz, pad_mode, resize_to
    ).view(-1, 4)
...
def _get_sz(x):
    ...
    return fastuple(
        getattr(x, "img_size", getattr(x, "sz", (x.shape[-1], x.shape[-2])))
    )
...
class CropPad(DisplayedTransform):
    ...
    def __init__(
        self,
        size: int
        | tuple,  # Size to crop or pad to, duplicated if one value is specified
...
class RandomCrop(RandTransform):
    ...
    def before_call(self, b, split_idx: int):  # Index of the train/valid dataset
...
class OldRandomCrop(CropPad):
    ...
    def before_call(self, b, split_idx):
        ...
        if not split_idx:
            self.tl = (
                random.randint(0, w - self.cp_size[0]),
                random.randint(0, h - self.cp_size[1]),
            )

# %% ../../nbs/09_vision.augment.ipynb 50
mk_class(
    "ResizeMethod",
    **{o: o.lower() for o in ["Squish", "Crop", "Pad"]},
...
class Resize(RandTransform):
    ...
    def __init__(
        ...
        resamples=(
            BILINEAR,
            NEAREST,
        ),  # Pillow `Image` resamples mode, resamples[1] for mask
        **kwargs
    ):
        size = _process_sz(size)
        store_attr()
    ...
    def encodes(self, x: Image.Image | TensorBBox | TensorPoint):
        ...
        if self.method == ResizeMethod.Squish:
            return x.crop_pad(
                ...
                resize_mode=self.mode_mask if isinstance(x, PILMask) else self.mode,
                resize_to=self.size,
            )
        ...
        m = (
            w / self.size[0]
            if op(w / self.size[0], h / self.size[1])
            else h / self.size[1]
        )
        cp_sz = (int(m * self.size[0]), int(m * self.size[1]))
        tl = fastuple(
            int(self.pcts[0] * (w - cp_sz[0])), int(self.pcts[1] * (h - cp_sz[1]))
        )
        return x.crop_pad(
            cp_sz,
            tl,
...
class RandomResizedCrop(RandTransform):
    ...
    def __init__(
        ...
        resamples=(
            BILINEAR,
            NEAREST,
        ),  # Pillow `Image` resample mode, resamples[1] for mask
    ...
    def before_call(self, b, split_idx):  # Index of the train/valid dataset
...
def affine_coord(
    ...
    if add_dim:
        x = x[:, None]
    res = TensorImage.affine_coord(
        x.float(), mat, coord_tfm, sz, mode, pad_mode, align_corners
    ).long()
    if add_dim:
        res = res[:, 0]
...
def affine_coord(
    ...
    if mat is not None:
        mat = TensorPoint(mat)
        x = (x - mat[:, :, 2].unsqueeze(1)) @ torch.inverse(
            mat[:, :, :2].transpose(1, 2)
        )
...
def affine_coord(
    ...
    pnts = TensorPoint(pnts.view(bs, 4 * n, 2), img_size=sz).affine_coord(
        mat, coord_tfm, sz, mode, pad_mode
    )
...
class AffineCoordTfm(RandTransform):
    ...
    def __init__(
        self,
        aff_fs: callable
        | MutableSequence = None,  # Affine transformations function for a batch
        coord_fs: callable
        | MutableSequence = None,  # Coordinate transformations function for a batch
    ...
    def _encode(self, x, mode, reverse=False):
        coord_func = (
            None
            if len(self.coord_fs) == 0 or self.split_idx
            else partial(compose_tfms, tfms=self.coord_fs, reverse=reverse)
        )
        return x.affine_coord(
            self.mat,
            coord_func,
            sz=self.size,
            mode=mode,
...
class RandomResizedCropGPU(RandTransform):
    ...
    def __init__(
        ...
        ratio=(3 / 4, 4 / 3),  # Range of width over height of the output
        mode="bilinear",  # PyTorch `F.grid_sample` interpolation
        valid_scale=1.0,  # Scale of the crop for the validation set, in relation to image area
        max_scale=1.0,  # Maximum scale of the crop, in relation to image area
    ...
    def before_call(self, b, split_idx):
        ...
        if split_idx:
            self.cp_size = (
                int(self.cp_size[0] * self.valid_scale),
                int(self.cp_size[1] * self.valid_scale),
            )
    ...
    def _encode(self, x, mode):
        x = x[
            ...,
            self.tl[0] : self.tl[0] + self.cp_size[0],
            self.tl[1] : self.tl[1] + self.cp_size[1],
        ]
...
def flip_mat(
    ...
    mask = x.new_ones(x.size(0)) - 2 * _draw_mask(
        x, _def_draw, draw=draw, p=p, batch=batch
    )
...
def _get_default(x, mode=None, pad_mode=None):
    ...
    if pad_mode is None:
        pad_mode = (
            PadMode.Zeros
            if isinstance(x, (TensorPoint, TensorBBox))
            else PadMode.Reflection
        )
...
@patch
def flip_batch(
    x: TensorImage | TensorMask | TensorPoint | TensorBBox,
...
def dihedral_mat(
    ...
    idx = _draw_mask(
        x, _def_draw_b if batch else _def_draw, draw=draw, p=p, batch=batch
    ).long()
...
def dihedral_batch(
    ...
    return x.affine_coord(
        mat=mat, sz=size, mode=mode, pad_mode=pad_mode, align_corners=align_corners
    )
...
class Dihedral(AffineCoordTfm):
    ...
    def __init__(
        ...
        draw: int
        | MutableSequence
        | callable = None,  # Custom dihedrals instead of random
        size: int | tuple = None,  # Output size, duplicated if one value is specified
        mode: str = "bilinear",  # PyTorch `F.grid_sample` interpolation
        pad_mode=PadMode.Reflection,  # A `PadMode`
        batch=False,  # Apply identical dihedral to entire batch
...
def rotate_mat(
    ...
    thetas = (
        _draw_mask(x, _def_draw_b if batch else _def_draw, draw=draw, p=p, batch=batch)
        * math.pi
        / 180
    )
    return affine_mat(
        thetas.cos(), thetas.sin(), t0(thetas), -thetas.sin(), thetas.cos(), t0(thetas)
    )
...
class Rotate(AffineCoordTfm):
    ...
    def __init__(
        ...
        draw: int
        | MutableSequence
        | callable = None,  # Custom rotates instead of random
        size: int | tuple = None,  # Output size, duplicated if one value is specified
        mode: str = "bilinear",  # PyTorch `F.grid_sample` interpolation
        pad_mode=PadMode.Reflection,  # A `PadMode`
        align_corners: bool = True,  # PyTorch `F.grid_sample` align_corners
...
def zoom_mat(
    ...
    draw_x: float
    | MutableSequence
    | callable = None,  # User defined center of the zoom in x
    draw_y: float
    | MutableSequence
    | callable = None,  # User defined center of the zoom in y
    ...
    s = 1 / _draw_mask(
        x, _def_draw_b if batch else _def_draw, draw=draw, p=p, neutral=1.0, batch=batch
    )
...
class Zoom(AffineCoordTfm):
    ...
    def __init__(
        ...
        draw: float
        | MutableSequence
        | callable = None,  # User defined scale of the zoom
        draw_x: float
        | MutableSequence
        | callable = None,  # User defined center of the zoom in x
        draw_y: float
        ...
        super().__init__(
            aff_fs, size=size, mode=mode, pad_mode=pad_mode, align_corners=align_corners
        )
...
class _WarpCoord:
    ...
    def __call__(self, x, invert=False):
        coeffs = (
            find_coeffs(self.targ_pts, self.orig_pts)
            if invert
            else find_coeffs(self.orig_pts, self.targ_pts)
        )
...
class Warp(AffineCoordTfm):
    ...
    def __init__(
        ...
        draw_x: float
        | MutableSequence
        | callable = None,  # User defined warping magnitude in x
        draw_y: float
        | MutableSequence
        | callable = None,  # User defined warping magnitude in y
        size: int | tuple = None,  # Output size, duplicated if one value is specified
        mode: str = "bilinear",  # PyTorch `F.grid_sample` interpolation
        pad_mode=PadMode.Reflection,  # A `PadMode`
        ...
        store_attr()
        coord_fs = _WarpCoord(
            magnitude=magnitude, p=p, draw_x=draw_x, draw_y=draw_y, batch=batch
        )
        super().__init__(
            coord_fs=coord_fs,
            size=size,
...
class SpaceTfm(RandTransform):
    ...
    def compose(self, tfm: callable):  # Transformation function to compose
...
class LightingTfm(SpaceTfm):
    ...
    def __init__(
        self,
        fs: callable
        | MutableSequence,  # Transformation functions applying in logit space,
        **kwargs
    ):
...
class _BrightnessLogit:
    ...
    def _def_draw(self, x):
        if not self.batch:
            return x.new_empty(x.size(0)).uniform_(
                0.5 * (1 - self.max_lighting), 0.5 * (1 + self.max_lighting)
            )
        return x.new_zeros(x.size(0)) + random.uniform(
            0.5 * (1 - self.max_lighting), 0.5 * (1 + self.max_lighting)
        )

    def before_call(self, x):
        self.change = _draw_mask(
            x, self._def_draw, draw=self.draw, p=self.p, neutral=0.5, batch=self.batch
        )
...
class Brightness(LightingTfm):
    def __init__(
        ...
        draw: float
        | MutableSequence
        | callable = None,  # User defined behavior of batch transformation
...
class _ContrastLogit:
    ...
    def _def_draw(self, x):
        if not self.batch:
            res = x.new_empty(x.size(0)).uniform_(
                math.log(1 - self.max_lighting), -math.log(1 - self.max_lighting)
            )
        else:
            res = x.new_zeros(x.size(0)) + random.uniform(
                math.log(1 - self.max_lighting), -math.log(1 - self.max_lighting)
            )
    ...
    def before_call(self, x):
        self.change = _draw_mask(
            x, self._def_draw, draw=self.draw, p=self.p, neutral=1.0, batch=self.batch
        )
...
def grayscale(x):
    ...
    return (
        x * torch.tensor([0.2989, 0.5870, 0.1140], device=x.device)[..., None, None]
    ).sum(1)[:, None]
...
def rgb2hsv(img: Tensor):  # Batch of images `Tensor`in RGB
...
class _Hue:
    ...
    def before_call(self, x):
        self.change = _draw_mask(
            x, self._def_draw, draw=self.draw, p=self.p, neutral=0.0, batch=self.batch
        )
...
def cutout_gaussian(
    x: Tensor, areas: list  # Input image  # List of areas to cutout. Order rl,rh,cl,ch
):
