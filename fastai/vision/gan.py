...
class GANModule(Module):
    ...
    def __init__(
        ...
        gen_mode: None
        | bool = False,  # Whether the GAN should be set to generator mode
    ):
...
def basic_critic(
    ...
    **kwargs
    ...
    layers += [
        ConvLayer(cur_ftrs, cur_ftrs, 3, 1, norm_type=norm_type, **kwargs)
        for _ in range(n_extra_layers)
    ]
    while cur_size > 4:
        layers.append(
            ConvLayer(cur_ftrs, cur_ftrs * 2, 4, 2, 1, norm_type=norm_type, **kwargs)
        )
        cur_ftrs *= 2
        cur_size //= 2
...
def basic_generator(
    ...
    **kwargs
    ...
    layers = [
        AddChannels(2),
        ConvLayer(in_sz, cur_ftrs, 4, 1, transpose=True, **kwargs),
    ]
    cur_size = 4
    while cur_size < out_size // 2:
        layers.append(
            ConvLayer(cur_ftrs, cur_ftrs // 2, 4, 2, 1, transpose=True, **kwargs)
        )
        cur_ftrs //= 2
        cur_size *= 2
    layers += [
        ConvLayer(cur_ftrs, cur_ftrs, 3, 1, 1, transpose=True, **kwargs)
        for _ in range(n_extra_layers)
    ]
...
_conv_args = dict(
    act_cls=partial(nn.LeakyReLU, negative_slope=0.2), norm_type=NormType.Spectral
)
...
class GANLoss(GANModule):
    ...
    def generator(self, output, target):  # Generator outputs  # Real images
...
def accuracy_thresh_expand(
    ...
    return (
        ((y_pred > thresh).byte() == y_true[:, None].expand_as(y_pred).byte())
        .float()
        .mean()
    )
...
class GANTrainer(Callback):
    ...
    def __init__(
        ...
        self.gen_loss, self.crit_loss = AvgSmoothLoss(beta=beta), AvgSmoothLoss(
            beta=beta
        )
...
class GANDiscriminativeLR(Callback):
    ...
    def before_batch(self):
        ...
        if not self.learn.gan_trainer.gen_mode and self.training:
            self.learn.opt.set_hyper(
                "lr", self.learn.opt.hypers[0]["lr"] * self.mult_lr
            )
...
def gan_loss_from_func(
    ...
    weights_gen: None
    | MutableSequence
    | tuple = None,  # Weights for the generator and critic loss function
):
...
class GANLearner(Learner):
    ...
    def __init__(
        ...
        crit_loss_func: callable,  # Critic loss function
        switcher: Callback
        | None = None,  # Callback for switching between generator and critic training, defaults to `FixedGANSwitcher`
        gen_first: bool = False,  # Whether we start with generator training
        ...
        **kwargs
        ...
        trainer = GANTrainer(
            clip=clip, switch_eval=switch_eval, gen_first=gen_first, show_img=show_img
        )
        cbs = L(cbs) + L(trainer, switcher)
        metrics = L(metrics) + L(*LossMetrics("gen_loss,crit_loss"))
        super().__init__(
            dls, gan, loss_func=loss_func, cbs=cbs, metrics=metrics, **kwargs
        )
    ...
    def from_learners(
        ...
        **kwargs
        ...
        losses = gan_loss_from_func(
            gen_learn.loss_func, crit_learn.loss_func, weights_gen=weights_gen
        )
        return cls(
            gen_learn.dls,
            gen_learn.model,
    ...
    def wgan(
        ...
        critic: nn.Module,  # Critic model
        switcher: Callback
        | None = None,  # Callback for switching between generator and critic training, defaults to `FixedGANSwitcher(n_crit=5, n_gen=1)`
