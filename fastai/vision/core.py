...
def __repr__(x: Image.Image):
    return "<%s.%s image mode=%s size=%dx%d>" % (
...
def get_annotations(fname, prefix=None):
    ...
    id2images = {
        o["id"]: ifnone(prefix, "") + o["file_name"]
...
def _draw_rect(ax, b, color="white", text=None, text_size=14, hw=True, rev=False):
    ...
    if not hw:
        w, h = w - lx, h - ly
    patch = ax.add_patch(
        patches.Rectangle((lx, ly), w, h, fill=False, edgecolor=color, lw=2)
    )
...
class TensorBBox(TensorPoint):
    ...
    @classmethod
    def create(cls, x, img_size=None) -> None:
        return cls(tensor(x).view(-1, 4).float(), img_size=img_size)
...
class PointScaler(Transform):
    ...
    def _get_sz(self, x):
        return getattr(x, "img_size") if self.sz is None else self.sz
    ...
    def encodes(self, x: TensorPoint):
        return _scale_pnts(x, self._get_sz(x), self.do_scale, self.y_first)

    def decodes(self, x: TensorPoint):
        return _unscale_pnts(x.view(-1, 2), self._get_sz(x))
...
@MultiCategorize
def decodes(self, x: LabeledBBox):
    return x
