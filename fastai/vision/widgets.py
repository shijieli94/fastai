...
class ImagesCleaner:
    ...
    def set_fns(self, fns: list):  # Contains a path to each image
        ...
        self.widget.children = [
            VBox(
                [
                    ...
                    Dropdown(options=self.opts, layout={"width": "max-content"}),
                ]
            )
            for im in ims
        ]
...
def _get_iw_info(learn, ds_idx: int = 0) -> list:  # Index in `learn.dls`
    "For every image in `dls` `zip` it's `Path`, target and loss"
    dl = learn.dls[ds_idx].new(shuffle=False, drop_last=False)
    probs, targs, preds, losses = learn.get_preds(
        dl=dl, with_input=False, with_loss=True, with_decoded=True
    )
