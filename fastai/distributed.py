...
class DistributedDL(TfmdDL):
    ...
    def get_idxs(self):
        idxs = list(
            self.dl.get_idxs()
        )  # compute get_idxs in all ranks (we'll only use rank 0 but size must be consistent)
        ...
        idxs += (idxs * (self.n_padded // self.n))[
    ...
    def to_detach(self, b, cpu=True, gather=True):
        ...
        return (
            apply(_inner, b)
            if gather and all(hasattr(self, o) for o in ("i", "n", "n_padded"))
            else b
        )
...
def distrib_ctx(
    ...
    **kwargs
    ...
    try:
        ...
        if num_distrib():
            self.to_distributed(sync_bn, **kwargs)
        yield self
    finally:
