...
def subplots(
    ...
    **kwargs
    ...
    if figsize is None:
        h = (
            nrows * imsize if suptitle is None or imsize > 2 else nrows * imsize + 0.6
...
def tensor(x, *rest, **kwargs):
    ...
    res = (
        ...
        else torch.tensor(x, **kwargs)
        if isinstance(x, (tuple, list, numbers.Number))
        else _array2tensor(x, **kwargs)
        if isinstance(x, ndarray)
        else as_tensor(x.values, **kwargs)
...
def to_concat(xs, dim=0):
    ...
    try:
        ...
        return sum(
            [
                L(
                    retain_type(o_.index_select(dim, tensor(i)).squeeze(dim), xs[0])
...
class TensorBase(Tensor):
    ...
    def __reduce_ex__(self, proto):
        if _torch_version >= _torch_20:
            ...
            f = (
                ...
                if self.is_quantized
                else torch._utils._rebuild_tensor_v2
            )
    ...
    def new_tensor(self, size, dtype=None, device=None, requires_grad=False):
        ...
        return (
            ...
            .new_tensor(size, dtype=dtype, device=device, requires_grad=requires_grad)
            .as_subclass(cls)
        )
    ...
    def new_ones(self, data, dtype=None, device=None, requires_grad=False):
        ...
        return (
            self.as_subclass(Tensor)
            .new_ones(data, dtype=dtype, device=device, requires_grad=requires_grad)
            .as_subclass(cls)
        )
    ...
    def new(self, x=None):
        cls = type(self)
        res = (
            self.as_subclass(Tensor).new()
            if x is None
            else self.as_subclass(Tensor).new(x)
        )
    ...
    def clone(self, *, memory_format=None):
        cls = type(self)
        return (
            self.as_subclass(Tensor).clone(memory_format=memory_format).as_subclass(cls)
        )
...
@patch
def __init__(
    self: pd.DataFrame, data=None, index=None, columns=None, dtype=None, copy=None
):
...
def flatten_check(inp, targ):
    ...
    inp, targ = TensorBase(inp.contiguous()).view(-1), TensorBase(
        targ.contiguous()
    ).view(-1)
...
def show_image_batch(
    b, show=show_titled_image, items=9, cols=3, figsize=None, **kwargs
):
