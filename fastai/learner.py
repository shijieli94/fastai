...
def load_model(
    file, model, opt, with_opt=True, device=None, strict=True, **torch_load_kwargs
):
...
class Learner(GetAttr):
    ...
    def __init__(
        ...
        cbs: Callback
        | MutableSequence
        | None = None,  # `Callback`s to add to `Learner`
        metrics: callable
        | MutableSequence
        | None = None,  # `Metric`s to calculate on validation set
        path: str
        | Path
        | None = None,  # Parent directory to save, load, and export models. Defaults to `dls` `path`
...
class Recorder(Callback):
    ...
    def plot_loss(self, skip_start=5, with_valid=True):
        ...
        if with_valid:
            ...
            plt.plot(
                self.iters[idx:], L(self.values[idx:]).itemgot(valid_col), label="valid"
            )
            plt.legend()
...
class CastToTensor(Callback):
    ...
    def before_batch(self):
        self.learn.xb, self.learn.yb = _cast_tensor(self.learn.xb), _cast_tensor(
            self.learn.yb
        )

# %% ../nbs/13a_learner.ipynb 155
if CastToTensor not in defaults.callbacks:
    defaults.callbacks.append(CastToTensor)
