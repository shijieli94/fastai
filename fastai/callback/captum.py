...
class CaptumInterpretation:
    ...
    def get_baseline_img(self, img_tensor, baseline_type):
        ...
        if baseline_type == "gauss":
            baseline_img = (
                torch.rand(img_tensor.shape).to(self.dls.device) + img_tensor
            ) / 2
    ...
    def visualize(
        ...
        if metric not in self.supported_metrics:
            raise Exception(
                f"Metric {metric} is not supported. Currently {self.supported_metrics} are only supported"
            )
        ...
        attributions = self._get_attributions(
            enc_data,
            metric,
            n_steps,
            nt_type,
    ...
    def _viz(self, attributions, dec_data, metric):
        default_cmap = LinearSegmentedColormap.from_list(
            self.cmap_name, self.colors, N=self.N
        )
        _ = viz.visualize_image_attr_multiple(
            ...
            titles=[f"Original Image - ({dec_data[1]})", metric],
        )

    def _get_enc_dec_data(self, inp_data):
        dec_data = self.dls.after_item(inp_data)
        enc_data = self.dls.after_batch(
            to_device(self.dls.before_batch(dec_data), self.dls.device)
        )
    ...
    def _get_attributions(
        ...
        if metric == "IG":
            self._int_grads = (
                self._int_grads
                if hasattr(self, "_int_grads")
                else IntegratedGradients(self.model)
            )
            return self._int_grads.attribute(
                enc_data[0], baseline, target=enc_data[1], n_steps=200
            )
        elif metric == "NT":
            self._int_grads = (
                self._int_grads
            ...
            self._noise_tunnel = (
                self._noise_tunnel
                if hasattr(self, "_noise_tunnel")
                else NoiseTunnel(self._int_grads)
            )
            return self._noise_tunnel.attribute(
            ...
            self._occlusion = (
                self._occlusion
                if hasattr(self, "_occlusion")
                else Occlusion(self.model)
            )
...
def insights(x: CaptumInterpretation, inp_data, debug=True):
    ...
    _get_vocab = (
        lambda vocab: list(map(str, vocab)) if isinstance(vocab[0], bool) else vocab
    )
