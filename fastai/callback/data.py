...
class WeightedDL(TfmdDL):
    ...
    def __init__(self, dataset=None, bs=None, wgts=None, **kwargs):
        wgts = array([1.0] * len(dataset) if wgts is None else wgts)
...
def weighted_dataloaders(self: Datasets, wgts, bs=64, **kwargs):
    ...
    return self.dataloaders(
        bs=bs, dl_type=WeightedDL, dl_kwargs=({"wgts": wgts}, *xtra_kwargs), **kwargs
    )
...
def weighted_dataloaders(
    self: DataBlock, source, wgts, bs=64, verbose: bool = False, **kwargs
):
    "Create a weighted dataloader `WeightedDL` with `wgts` for the dataset"
    dss = self.datasets(source, verbose=verbose)
    if not hasattr(wgts, "__array__"):
        wgts = np.array(wgts)
    trn_wgts = wgts[dss.splits[0]]
    return dss.weighted_dataloaders(
        trn_wgts,
        bs=bs,
        after_batch=self.batch_tfms,
...
class PartialDL(TfmdDL):
    ...
    def __len__(self):
        ...
        return self.partial_n // self.bs + (
            0 if self.drop_last or self.partial_n % self.bs == 0 else 1
        )
...
def partial_dataloaders(self: FilteredBase, partial_n, bs=64, **kwargs):
    ...
    return self.dataloaders(
        bs=bs,
        dl_type=PartialDL,
        dl_kwargs=({"partial_n": partial_n}, *xtra_kwargs),
        **kwargs
    )
