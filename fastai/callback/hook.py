...
class Hook:
    ...
    def __init__(
        self, m, hook_func, is_forward=True, detach=True, cpu=False, gather=False
    ):
    ...
    def hook_fn(self, module, input, output):
        ...
        if self.detach:
            input, output = to_detach(
                input, cpu=self.cpu, gather=self.gather
            ), to_detach(output, cpu=self.cpu, gather=self.gather)
...
class Hooks:
    ...
    _docs = dict(
        stored="The states saved in each hook.",
...
class HookCallback(Callback):
    ...
    def before_fit(self):
        ...
        if self.modules is None:
            self.modules = [
                ...
                if self.include_paramless or has_params(m)
            ]
    ...
    def _register(self):
        self.hooks = Hooks(
            self.modules, self.hook, self.is_forward, self.detach, self.cpu
        )
...
def layer_info(learn, *xb):
    ...
    def _track(m, i, o):
        ...
        same = any(
            (
                isinstance(x[0], torch.Tensor) and x[0].shape[1:] == x[1].shape
                for x in zip(i, o)
            )
        )
...
def module_summary(learn, *xb):
    ...
    res += "=" * n + "\n"
    res += (
        f"{'Layer (type)':<20} {'Output Shape':<20} {'Param #':<10} {'Trainable':<10}\n"
    )
    res += "=" * n
    ps, trn_ps, j = 0, 0, 0
...
class ActivationStats(HookCallback):
    ...
    def plot_layer_stats(self, idx):
        ...
        for o, ax, title in zip(
            self.layer_stats(idx), axs, ("mean", "std", "% near zero")
        ):
            ax.plot(o)
            ax.set_title(title)
