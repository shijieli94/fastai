...
def _write_projector_embedding(learn, writer, feat):
    lbls = (
        [learn.dl.vocab[l] for l in feat["lbl"]]
        if getattr(learn.dl, "vocab", None)
        else None
    )
    vecs = feat["vec"].squeeze()
    writer.add_embedding(
        vecs, metadata=lbls, label_img=feat["img"], global_step=learn.train_iter
    )
...
def _add_projector_features(learn, hook, feat):
    ...
    feat["vec"] = (
        hook.stored if first_epoch else torch.cat((feat["vec"], hook.stored), 0)
    )
...
def projector_word_embeddings(
    learn=None, layer=None, vocab=None, limit=-1, start=0, log_dir=None
):
    ...
    writer.add_embedding(
        emb[start:end], metadata=vocab[start:end], label_img=img[start:end]
    )
    writer.close()
...
def tensorboard_log(
    x: TensorImage,
    y: TensorImageBase | TensorPoint | TensorBBox,
    samples,
    outs,
    writer,
    step,
