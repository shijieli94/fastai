...
def fit_one_cycle(
    ...
    self.fit(
        n_epoch,
        cbs=ParamScheduler(scheds) + L(cbs),
        reset_opt=reset_opt,
        wd=wd,
...
def slide(
    ...
    while (l_idx >= -len(losses)) and (
        abs(loss_grad[r_idx] - loss_grad[l_idx]) > thresh
    ):
        local_min_lr = lrs[l_idx]
        r_idx -= 1
        l_idx -= 1
...
@patch
def plot_lr_find(
    self: Recorder, skip_end=5, return_fig=True, suggestions=None, nms=None, **kwargs
):
...
mk_class(
    "SuggestionMethod",
    **{o.__name__.capitalize(): o for o in [valley, slide, minimum, steep]},
...
def lr_find(
    ...
    stop_div=True,
    show_plot=True,
    suggest_funcs=(SuggestionMethod.Valley),
):
    ...
    if suggest_funcs is not None:
        ...
        for func in tuplify(suggest_funcs):
            nms.append(
                func.__name__ if not isinstance(func, partial) else func.func.__name__
            )  # deal with partials
        ...
        for lr, pnt in _suggestions:
            ...
            pnts.append(pnt)
        if show_plot:
            self.recorder.plot_lr_find(suggestions=pnts, nms=nms)
