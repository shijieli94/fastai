...
class TrackerCallback(Callback):
    ...
    def __init__(
        ...
        if comp == np.less:
            min_delta *= -1
        self.monitor, self.comp, self.min_delta, self.reset_on_fit, self.best = (
            monitor,
            comp,
            min_delta,
            reset_on_fit,
...
class ReduceLROnPlateau(TrackerCallback):
    ...
    def __init__(
        ...
        min_delta=0.0,  # minimum delta between the last monitor value and the best monitor value.
        patience=1,  # number of epochs to wait when training has not improved model.
        factor=10.0,  # the denominator to divide the learning rate by, when reducing the learning rate.
        ...
        if self.new_best:
            ...
            if self.wait >= self.patience:
                ...
                if self.opt.hypers[-1]["lr"] < old_lr:
                    print(
                        f'Epoch {self.epoch}: reducing lr to {self.opt.hypers[-1]["lr"]}'
                    )
