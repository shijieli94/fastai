...
class MixHandler(Callback):
    ...
    def __init__(
        self, alpha: float = 0.5  # Determine `Beta` distribution in range (0.,inf]
    ):
...
class MixUp(MixHandler):
    ...
    def __init__(
        self, alpha: float = 0.4  # Determine `Beta` distribution in range (0.,inf]
    ):
    ...
    def before_batch(self):
        ...
        self.learn.xb = tuple(
            L(xb1, self.xb).map_zip(
                torch.lerp, weight=unsqueeze(self.lam, n=nx_dims - 1)
            )
        )
        ...
        if not self.stack_y:
            ...
            self.learn.yb = tuple(
                L(self.yb1, self.yb).map_zip(
                    torch.lerp, weight=unsqueeze(self.lam, n=ny_dims - 1)
                )
            )
...
class CutMix(MixHandler):
    ...
    def __init__(
        self, alpha: float = 1.0  # Determine `Beta` distribution in range (0.,inf]
    ):
        super().__init__(alpha)
