...
class MixedPrecision(Callback):
    ...
    def __init__(
        self,
        amp_mode: str
        | AMPMode = AMPMode.FP16,  # Mixed Precision training mode. Supports fp16 and bf16.
        **kwargs,
    ):
    ...
    def before_fit(self):
        if self.amp_mode == AMPMode.BF16:
            if torch.cuda.is_available() and not torch.cuda.is_bf16_supported():
                raise ValueError(
                    "Unsupported GPU for bfloat16 mixed precision training"
                )
...
def get_master(
    ...
    if flat_master:
        ...
        master_params = [
            [
                nn.Parameter(param.data.clone().float().detach(), requires_grad=True)
                for param in pg
            ]
            for pg in model_params
        ]
...
def copy_clone(d):
    return {
        k: (v.detach().clone().float() if isinstance(v, Tensor) else v)
        for k, v in d.items()
    }
...
class NonNativeMixedPrecision(Callback):
    ...
    def __init__(
        ...
        max_loss_scale: float = 2.0**24,  # Starting value for dynamic loss scaling
    ...
    _docs = dict(
        before_fit="Put the model in FP16 and prepare the two copies of the parameters",
