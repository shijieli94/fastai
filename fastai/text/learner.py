...
def match_embeds(
    ...
    old_o2i = (
        old_vocab.o2i
        if hasattr(old_vocab, "o2i")
        else {w: i for i, w in enumerate(old_vocab)}
    )
...
def load_ignore_keys(model, wgts: dict) -> tuple:  # Model architecture  # Model weights
...
def load_model_text(
    ...
    device: int
    | str
    | torch.device = None,  # Sets the device, uses 'cpu' if unspecified
    ...
    if hasopt and ifnone(with_opt, True):
        ...
        warn("Saved file doesn't contain an optimizer state.")
...
class TextLearner(Learner):
    ...
    def save_encoder(self, file: str):  # Filename for `Encoder`
    ...
    def load_encoder(
        ...
        device: int
        | str
        | torch.device = None,  # Device used to load, defaults to `dls` device
    ):
    ...
    def load_pretrained(
        ...
        if "model" in wgts:
            wgts = wgts[
                "model"
            ]  # Just in case the pretrained model was saved with an optimizer
...
class LMLearner(TextLearner):
    ...
    def predict(
        ...
        for _ in (
            range(n_words) if no_bar else progress_bar(range(n_words), leave=False)
        ):
            with self.no_bar():
    ...
    def get_preds(self, concat_dim=1, **kwargs):
        return super().get_preds(concat_dim=1, **kwargs)
    ...
    **kwargs,
    ...
    if pretrained or pretrained_fnames:
        if pretrained_fnames is not None:
            ...
            try:
                fnames = [
                    list(model_path.glob(f"*.{ext}"))[0] for ext in ["pth", "pkl"]
                ]
            except IndexError:
...
def show_results(x: LMTensorText, y, samples, outs, ctxs=None, max_n=10, **kwargs):
    ...
    for i, l in enumerate(["input", "target"]):
        ctxs = [
            b.show(ctx=c, label=l, **kwargs)
            for b, c, _ in zip(samples.itemgot(i), ctxs, range(max_n))
        ]
    ctxs = [
        b.show(ctx=c, label="pred", **kwargs)
        for b, c, _ in zip(outs.itemgot(0), ctxs, range(max_n))
    ]
...
@typedispatch
def show_results(
    x: TensorText, y, samples, outs, ctxs=None, max_n=10, trunc_at=150, **kwargs
):
    if ctxs is None:
        ctxs = get_empty_df(min(len(samples), max_n))
...
def plot_top_losses(
    ...
    **kwargs
    ...
    outs = L(
        o + (TitledFloat(r.max().item()), TitledFloat(l.item()))
        for o, r, l in zip(outs, raws, losses)
    )
