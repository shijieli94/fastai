...
class WeightDropout(Module):
    ...
    def __init__(
        ...
        layer_names: str
        | MutableSequence = "weight_hh_l0",  # Name(s) of the parameters to apply dropout to
    ):
...
class AWD_LSTM(Module):
    ...
    def _one_hidden(self, l):
        ...
        return (
            one_param(self).new_zeros(self.n_dir, self.bs, nh),
            one_param(self).new_zeros(self.n_dir, self.bs, nh),
        )
    ...
    def _change_one_hidden(self, l, bs):
        if self.bs < bs:
            nh = (self.n_hid if l != self.n_layers - 1 else self.emb_sz) // self.n_dir
            return tuple(
                torch.cat([h, h.new_zeros(self.n_dir, bs - self.bs, nh)], dim=1)
                for h in self.hidden[l]
            )
        if self.bs > bs:
            return (
                self.hidden[l][0][:, :bs].contiguous(),
                self.hidden[l][1][:, :bs].contiguous(),
            )
...
def awd_lstm_lm_split(model):
    ...
    groups = L(
        groups + [nn.Sequential(model[0].encoder, model[0].encoder_dp, model[1])]
    )
