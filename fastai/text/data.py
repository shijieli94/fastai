...
class Numericalize(Transform):
    ...
    def __init__(self, vocab=None, min_freq=3, max_vocab=60000, special_toks=None):
        ...
        self.o2i = (
            None
            if vocab is None
            else defaultdict(int, {v: k for k, v in enumerate(vocab)})
        )
    ...
    def setups(self, dsets):
        ...
        if self.vocab is None:
            count = (
                dsets.counter
                if getattr(dsets, "counter", None) is not None
                else Counter(p for o in dsets for p in o)
            )
            ...
            self.o2i = defaultdict(
                int, {v: k for k, v in enumerate(self.vocab) if v != "xxfake"}
            )
...
@typedispatch
def show_batch(
    x: LMTensorText, y, samples, ctxs=None, max_n=10, trunc_at=150, **kwargs
):
    samples = L((s[0].truncate(trunc_at), s[1].truncate(trunc_at)) for s in samples)
    return show_batch[TensorText](
        x, None, samples, ctxs=ctxs, max_n=max_n, trunc_at=None, **kwargs
    )

# %% ../../nbs/31_text.data.ipynb 39
class Pad_Input(ItemTransform):
    def encodes(
        self, samples, pad_idx=1, pad_fields=0, pad_first=False, backwards=False
    ):
...
def pad_input_chunk(samples, n_inp=1, **kwargs):
    ...
    padeds = [
        [pad_chunk(s[n], pad_len=max_len, **kwargs) for n in range(n_inp)]
        for s in samples
    ]
...
class SortedDL(TfmdDL):
    ...
    def __init__(self, dataset, sort_func=None, res=None, **kwargs):
        ...
        self.res = (
            [self.sort_func(self.do_item(i)) for i in range_of(self.dataset)]
            if res is None
            else res
        )
        if len(self.res) > 0:
...
class TextBlock(TransformBlock):
    ...
    def __init__(
        self, tok_tfm, vocab=None, is_lm=False, seq_len=72, backwards=False, **kwargs
    ):
        ...
        return super().__init__(
            ...
            if is_lm
            else {"before_batch": Pad_Chunk(seq_len=seq_len)},
        )
...
class TextDataLoaders(DataLoaders):
        ...
        **kwargs
        ...
        blocks = [
            TextBlock.from_folder(
                path, text_vocab, is_lm, seq_len, backwards, tok=tok_tfm
            )
        ]
        if not is_lm:
            blocks.append(CategoryBlock(vocab=vocab))
        get_items = (
            partial(get_text_files, folders=[train, valid])
            if valid_pct is None
            else get_text_files
        )
        dblock = DataBlock(
    ...
    def from_df(
        ...
        **kwargs
        ...
        blocks = [
            TextBlock.from_df(
                text_col, text_vocab, is_lm, seq_len, backwards, tok=tok_tfm
            )
        ]
        if y_block is None and not is_lm:
            blocks.append(
                MultiCategoryBlock
                if is_listy(label_col) and len(label_col) > 1
                else CategoryBlock
            )
...
TextDataLoaders.from_csv = delegates(to=TextDataLoaders.from_df)(
