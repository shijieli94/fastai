...
def Flatten(self, x):
    ...
    return (
        x.view(-1) if self.full else x.view(x.size(0), -1)
    )  # Removed cast to Tensorbase
...
class PoolFlatten(nn.Sequential):
    ...
    def __init__(self, pool_type=PoolType.Avg):
        super().__init__(adaptive_pool(pool_type)(1), Flatten())
...
def InstanceNorm(nf, ndim=2, norm_type=NormType.Instance, affine=True, **kwargs):
    ...
    return _get_norm(
        ...
        nf,
        ndim,
        zero=norm_type == NormType.InstanceZero,
        affine=affine,
        **kwargs
    )
...
def init_default(m, func=nn.init.kaiming_normal_):
    ...
    if func and hasattr(m, "weight"):
        func(m.weight)
    with torch.no_grad():
        nested_callable(m, "bias.fill_")(0.0)
    return m
...
class ConvLayer(nn.Sequential):
    ...
    def __init__(
        ...
        **kwargs
        ...
        conv = conv_func(
            ni, nf, kernel_size=ks, bias=bias, stride=stride, padding=padding, **kwargs
        )
...
class PooledSelfAttention2d(Module):
    ...
    def __init__(self, n_channels):
        ...
        self.query, self.key, self.value = [
            self._conv(n_channels, c)
            for c in (n_channels // 8, n_channels // 8, n_channels // 2)
        ]
    ...
    def _conv(self, n_in, n_out):
        return ConvLayer(
            n_in, n_out, ks=1, norm_type=NormType.Spectral, act_cls=None, bias=False
        )

    def forward(self, x):
        n_ftrs = x.shape[2] * x.shape[3]
        f = self.query(x).view(-1, self.n_channels // 8, n_ftrs)
        g = F.max_pool2d(self.key(x), [2, 2]).view(
            -1, self.n_channels // 8, n_ftrs // 4
        )
        h = F.max_pool2d(self.value(x), [2, 2]).view(
            -1, self.n_channels // 2, n_ftrs // 4
        )
        beta = F.softmax(torch.bmm(f.transpose(1, 2), g), -1)
        o = self.out(
            torch.bmm(h, beta.transpose(1, 2)).view(
    ...
    def __init__(
        ...
        if norm_type == NormType.Weight:
            ...
            layers[0][0].weight_g.data.copy_(
                ((layers[0][0].weight_v.data ** 2).sum(dim=[1, 2, 3]) ** 0.5)[
                    :, None, None, None
                ]
            )
        else:
    ...
    def __init__(self, filters, kernel_szs=None, strides=None, bn=True):
        ...
        layers = [
            ConvLayer(
                ...
                kernel_szs[i],
                stride=strides[i],
                norm_type=(NormType.Batch if bn and i < nl - 1 else None),
            )
            for i in range(nl)
        ]
    ...
    def __init__(
        ...
        **kwargs
        ...
        convpath = (
            [
                ConvLayer(
                    ni, nh2, ks, stride=stride, groups=ni if dw else groups, **k0
                ),
            ...
            if expansion == 1
                ...
                ConvLayer(
                    nh1, nh2, ks, stride=stride, groups=nh1 if dw else groups, **k0
                ),
...
def SEResNeXtBlock(
    expansion, ni, nf, groups=32, reduction=16, stride=1, base_width=4, **kwargs
):
...
def _stack_tups(tuples, stack_dim=1):
    ...
    return tuple(
        torch.stack([t[i] for t in tuples], dim=stack_dim) for i in range_of(tuples[0])
    )
...
class TimeDistributed(Module):
    ...
    def forward(self, *tensors, **kwargs):
        ...
        if self.low_mem or self.tdim != 1:
            ...
            out = self.module(
                *[x.view(bs * seq_len, *x.shape[2:]) for x in tensors], **kwargs
            )
...
def mish(x, inplace=False):
    return F.mish(x, inplace=inplace)
...
for o in swish, Swish, SwishJit, mish, Mish, MishJit:
    o.__default_init__ = kaiming_uniform_
...
def flatten_model(m):
    ...
    return (
        sum(map(flatten_model, children_and_parameters(m)), [])
        if has_children(m)
        else [m]
    )
...
def in_channels(m):
    ...
    try:
        return next(
            ...
            if nested_attr(l, "weight.ndim", -1) == 4
        )
    except StopIteration as e:
        e.args = ["No weight layer"]
        raise
