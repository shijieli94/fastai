...
class Interpretation:
    ...
    def __getitem__(self, idxs):
        ...
        tmp_dl = self.learn.dls.test_dl(
            items, with_labels=True, process=not isinstance(self.dl, TabDataLoader)
        )
        inps, preds, targs, decoded = self.learn.get_preds(
            dl=tmp_dl,
            with_input=True,
            with_loss=False,
            with_decoded=True,
    ...
    def plot_top_losses(
        ...
        **kwargs
        ...
        if its is not None:
            plot_top_losses(
                x, y, its, outs.itemgot(slice(len(inps), None)), preds, losses, **kwargs
            )
        # TODO: figure out if this is needed
        # its None means that a batch knows how to show itself as a whole, so we pass x, x1
        # else: show_results(x, x1, its, ctxs=ctxs, max_n=max_n, **kwargs)

    def show_results(self, idxs: list, **kwargs):  # Indices of predictions and targets
...
class ClassificationInterpretation(Interpretation):
    ...
    def most_confused(self, min_val=1):
        ...
        res = [
            (self.vocab[i], self.vocab[j], cm[i, j])
    ...
    def print_classification_report(self):
        ...
        print(
            skm.classification_report(
                t, d, labels=list(self.vocab.o2i.values()), target_names=names
            )
        )
