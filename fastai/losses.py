...
class BaseLoss:
    ...
    def _contiguous(self, x: Tensor) -> TensorBase:
        ...
        return (
            TensorBase(x.transpose(self.axis, -1).contiguous())
    ...
    def __call__(
        ...
        **kwargs,
        ...
        return self.func.__call__(
            inp, targ.view(-1) if self.flatten else targ, **kwargs
        )
...
class CrossEntropyLossFlat(BaseLoss):
    ...
    def __init__(self, *args, axis: int = -1, **kwargs):  # Class axis
...
class BCEWithLogitsLossFlat(BaseLoss):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if (
            kwargs.get("pos_weight", None) is not None
            and kwargs.get("flatten", None) is True
        ):
            raise ValueError(
...
@use_kwargs_dict(reduction="mean")
def L1LossFlat(
    *args, axis=-1, floatify=True, **kwargs  # Class axis  # Convert `targ` to `float`
):
    "Same as `nn.L1Loss`, but flattens input and target."
    return BaseLoss(
        nn.L1Loss, *args, axis=axis, floatify=floatify, is_2d=False, **kwargs
    )
...
class DiceLoss:
    ...
    def __call__(self, pred: Tensor, targ: Tensor) -> Tensor:
        ...
        ), "input and target dimensions differ, DiceLoss expects non one-hot targs"
    ...
    def _one_hot(
        ...
        return torch.stack(
            [torch.where(x == c, 1, 0) for c in range(classes)], axis=axis
        )
