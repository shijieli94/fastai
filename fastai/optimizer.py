...
class _BaseOptimizer:
    ...
    def all_params(
        self,
        n: slice
        | int = slice(None),  # Extended slicing over the optimizer `param_lists`
    ...
    def freeze_to(self, n: int):  # Freeze up to `n` layers
    ...
    def _set_hyper(self, k, v):  # Hyperparameter key  # Hyperparameter value
    ...
    @param_groups.setter
    def param_groups(
        self, v: dict  # List of dicts to set `params` and other hyper parameters
    ):
...
def _update(state: dict, new=None):  # New values to update `state` dict
    if new is None:
        return state
...
class Optimizer(_BaseOptimizer):
    ...
    def __init__(
        ...
        **defaults  # Hyper parameters default values
        ...
        if "train_bn" in defaults.keys():
            ...
            warn(
                "Setting `train_bn` in `Optimizer` has no effect. Set `train_bn` on `Learner` init instead"
            )
        params = L(params)
        self.cbs, self.state = L(cbs), defaultdict(dict)
        defaults = merge(*self.cbs.attrgot("defaults"), defaults)
        self.param_lists = (
            L(L(p) for p in params) if isinstance(params[0], (L, list)) else L([params])
        )
    ...
    def step(self, closure=None):
        if closure is not None:
            raise NotImplementedError(
                "fastai optimizers currently do not support closure"
            )
    ...
    def load_state_dict(
        self, sd: dict  # State dict with `hypers` and `state` to load on the optimizer
    ):
...
weight_decay.defaults = dict(wd=0.0)
...
def RMSProp(
    ...
    "A RMSProp `Optimizer`"
    cbs = [weight_decay] if decouple_wd else [l2_reg]
    cbs += [average_sqr_grad] if mom == 0.0 else [average_grad, average_sqr_grad]
...
def qhadam_step(
    ...
    debias2 = debias(sqr_mom, 1 - sqr_mom, step)
    p.data.addcdiv_(
        ((1 - nu_1) * p.grad.data) + (nu_1 * (grad_avg / debias1)),
...
def QHAdam(
    ...
    cbs += [
        partial(average_grad, dampening=True),
        partial(average_sqr_grad, dampening=True),
        step_stat,
        qhadam_step,
    ]
    return Optimizer(
...
larc_layer_lr.defaults = dict(trust_coeff=0.02, wd=0.0, eps=1e-8)
...
def Larc(
    ...
    return Optimizer(
        params, cbs, lr=lr, mom=mom, trust_coeff=trust_coeff, eps=eps, wd=wd
    )
...
lamb_step._defaults = dict(eps=1e-6, wd=0.0)
...
class Lookahead(Optimizer, GetAttr):
    ...
    def _init_state(self):
        self.count, self.slow_weights = 0, None

    def _copy_weights(self):
        self.slow_weights = L(
            L(p.clone().detach() for p in pg) for pg in self.param_lists
        )

    @property
    def param_lists(self):
        return self.opt.param_lists

    @param_lists.setter
    def param_lists(self, v):
        self.opt.param_lists = v
...
def ranger(
    ...
    **kwargs
    ...
    return Lookahead(
        RAdam(params, lr=lr, mom=mom, wd=wd, eps=eps, **kwargs), k=k, alpha=alpha
    )
...
class OptimWrapper(_BaseOptimizer, GetAttr):
    ...
    def __init__(
        self,
        params: Tensor
        | Iterable = None,  # Model parameters. Don't set if using a built optimizer
        opt: callable
        | torch.optim.Optimizer = None,  # A torch optimizer constructor, or an already built optimizer
        ...
        **kwargs
        ...
        if params is None and opt is None:
            raise ValueError("Both `params` and `opt` cannot be None.")
        if callable(opt):
            if convert_groups:
                params = L(params)
                convert_groups = isinstance(params[0], (L, list))
            self.opt = (
                opt(_convert_params(params), **kwargs)
                if convert_groups
                else opt(params, **kwargs)
            )
        else:
            if params is not None:
                raise ValueError(
                    "Tried using both `params` and a built optimizer. Just pass in `opt`."
                )
            self.opt = opt
        if hp_map is None:
        ...
        self.fwd_map = {
            ...
            for k in detuplify_pg(self.opt.param_groups[0]).keys()
        }
    ...
    def hypers(self):
        return [
            {
                self.fwd_map.get(k, k): v
                for k, v in detuplify_pg(pg).items()
                if k != "params"
            }
