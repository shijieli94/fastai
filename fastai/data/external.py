...
def fastai_cfg() -> Config:  # Config that contains default download paths for `data`, `model`, `storage` and `archive`
    ...
    return Config(
        Path(os.getenv("FASTAI_HOME", "~/.fastai")),
        "config.ini",
        create=dict(data="data", archive="archive", storage="tmp", model="models"),
    )
...
class URLs:
    ...
    def path(
        ...
        fname = url.split("/")[-1]
        local_path = (
            URLs.LOCAL_PATH / ("models" if c_key == "model" else "data") / fname
        )
...
def untar_data(
    ...
    d = FastDownload(
        fastai_cfg(), module=fastai.data, archive=archive, data=data, base=base
    )
