...
class TransformBlock:
    ...
    def __init__(
        ...
        self.dl_type, self.dls_kwargs = dl_type, (
            {} if dls_kwargs is None else dls_kwargs
        )
...
def MultiCategoryBlock(
    ...
    tfm = (
        EncodedMultiCategorize(vocab=vocab)
        if encoded
        else [MultiCategorize(vocab=vocab, add_na=add_na), OneHotEncode]
    )
...
class DataBlock:
        ...
        **kwargs,
        ...
        if self.get_x:
            if len(L(self.get_x)) != self.n_inp:
                raise ValueError(
                    f"get_x contains {len(L(self.get_x))} functions, but must contain {self.n_inp} (one for each input)\n{self._msg}"
                )
        ...
        if self.get_y:
            ...
            if len(L(self.get_y)) != n_targs:
                raise ValueError(
                    f"get_y contains {len(L(self.get_y))} functions, but must contain {n_targs} (one for each target)\n{self._msg}"
                )
    ...
    def from_columns(
        ...
        **kwargs,
        ...
        if getters is None:
            getters = L(
                ItemGetter(i) for i in range(2 if blocks is None else len(L(blocks)))
            )
    ...
    def datasets(
        ...
        pv(
            f"{len(splits)} datasets of sizes {','.join([str(len(s)) for s in splits])}",
            verbose,
        )
        return Datasets(
            items,
            tfms=self._combine_type_tfms(),
            splits=splits,
    ...
    def dataloaders(
        ...
        **kwargs,
        ...
        return dsets.dataloaders(
            path=path, after_item=self.item_tfms, after_batch=self.batch_tfms, **kwargs
        )

    _docs = dict(
        new="Create a new `DataBlock` with other `item_tfms` and `batch_tfms`",
