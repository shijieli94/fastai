...
def ImageGetter(suf="", recurse=True, folders=None):
    ...
    def _inner(o, recurse=recurse, folders=folders):
        return get_image_files(o / suf, recurse, folders)
...
class AttrGetter(ItemTransform):
    ...
    def __init__(self, nm, default=None):
        store_attr()

    def encodes(self, x):
        return getattr(x, self.nm, self.default)
...
def TrainTestSplitter(
    test_size=0.2, random_state=None, stratify=None, train_size=None, shuffle=True
):
...
class ColReader(DisplayedTransform):
    ...
    def _do_one(self, r, c):
        o = (
            r[c]
            if isinstance(c, int) or not c in getattr(r, "_fields", [])
            else getattr(r, c)
        )
...
class CategoryMap(CollBase):
    ...
    def __init__(self, col, sort=True, add_na=False, strict=False):
        ...
        self.o2i = (
            defaultdict(int, self.items.val2idx())
            if add_na
...
class Categorize(DisplayedTransform):
    ...
    def encodes(self, o):
        try:
            ...
            raise KeyError(
                f"Label '{o}' was not included in the training dataset"
            ) from e
...
class OneHotEncode(DisplayedTransform):
    ...
    def setups(self, dsets):
        ...
        if not self.c:
            warn(
                "Couldn't infer the number of classes, please pass a value for `c` at init"
            )

    def encodes(self, o):
        return TensorMultiCategory(one_hot(o, self.c).float())

    def decodes(self, o):
        return one_hot_decode(o, None)
...
class EncodedMultiCategorize(Categorize):
    ...
    def encodes(self, o):
        return TensorMultiCategory(tensor(o).float())

    def decodes(self, o):
        return MultiCategory(one_hot_decode(o, self.vocab))
...
class RegressionSetup(DisplayedTransform):
    ...
    def decodes(self, o):
        return TitledFloat(o) if o.ndim == 0 else TitledTuple(o_.item() for o_ in o)
...
def get_c(dls):
    if getattr(dls, "c", False):
        return dls.c
    if nested_attr(dls, "train.after_item.c", False):
        return dls.train.after_item.c
    if nested_attr(dls, "train.after_batch.c", False):
...
class IntToFloatTensor(DisplayedTransform):
    ...
    def __init__(self, div=255.0, div_mask=1):
        store_attr()
    ...
    def encodes(self, o: TensorMask):
        return (o.long() / self.div_mask).long()

    def decodes(self, o: TensorImage):
        return ((o.clamp(0.0, 1.0) * self.div).long()) if self.div else o
...
class Normalize(DisplayedTransform):
    ...
    def setups(self, dl: DataLoader):
        if self.mean is None or self.std is None:
            x, *_ = dl.one_batch()
            self.mean, self.std = (
                x.mean(self.axes, keepdim=True),
                x.std(self.axes, keepdim=True) + 1e-7,
            )
