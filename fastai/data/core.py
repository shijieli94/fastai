...
class TfmdDL(DataLoader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            dataset, bs=bs, shuffle=shuffle, num_workers=num_workers, **kwargs
        )
        if do_setup:
            for nm in _batch_tfms:
    ...
    def _one_pass(self):
        ...
        self._n_inp = (
            1 if not isinstance(its, (list, tuple)) or len(its) == 1 else len(its) - 1
        )
    ...
    def decode(self, b):  # Batch to decode
    ...
    def show_results(
        ...
        **kwargs,
        ...
        res = (
            (x, x1, None, None)
            if its is None
            else (x, y, its, outs.itemgot(slice(self.n_inp, None)))
        )
        if not show:
            return res
    ...
    def to(self, device):  # Device to put `DataLoader` and transforms
        self.device = device
...
class DataLoaders(GetAttr):
    ...
    def __init__(
        ...
        *loaders,  # `DataLoader` objects to wrap
        ...
        if device is not None and (loaders != () and hasattr(loaders[0], "to")):
            self.device = device
    ...
    @device.setter
    def device(self, d):  # Device to put `DataLoaders`
        for dl in self.loaders:
            dl.to(d)
        self._device = d

    def to(self, device):  # Device to put `DataLoaders`
        self.device = device
        return self

    def _add_tfms(self, tfms, event, dl_idx):
        "Adds `tfms` to `event` on `dl`"
        if isinstance(dl_idx, str):
            dl_idx = 0 if (dl_idx == "train") else 1
    ...
    def add_tfms(
        ...
        if loaders is None:
            loaders = range(len(self.loaders))
        if not is_listy(loaders):
            loaders = listify(loaders)
        for loader in loaders:
            self._add_tfms(tfms, event, loader)

    def cuda(self):
        return self.to(device=default_device())
    ...
    def from_dsets(
        ...
        **kwargs
        ...
        tfms = {
            k: tuple(Pipeline(kwargs[k]) for i in range_of(ds))
        ...
        kwargs = merge(
            defaults,
            {
                k: tuplify(v, match=ds)
                for k, v in kwargs.items()
                if k not in _batch_tfms
            },
            tfms,
        )
...
class FilteredBase:
    ...
    def _new(self, items, **kwargs):
        return super()._new(items, splits=self.splits, **kwargs)
    ...
    def dataloaders(
        ...
        **kwargs
        ...
        if shuffle_train is not None:
            ...
            warnings.warn(
                "`shuffle_train` is deprecated. Use `shuffle` instead.",
                DeprecationWarning,
            )
        if device is None:
        ...
        dls = [dl] + [
            dl.new(
                self.subset(i), **merge(kwargs, def_kwargs, val_kwargs, dl_kwargs[i])
            )
...
class TfmdLists(FilteredBase, L, GetAttr):
    ...
    def subset(self, i):
        return self._new(self._get(self.splits[i]), split_idx=i)

    def _after_item(self, o):
        return self.tfms(o)
    ...
    def show(self, o, **kwargs):
        return self.tfms.show(o, **kwargs)

    def decode(self, o, **kwargs):
        return self.tfms.decode(o, **kwargs)

    def __call__(self, o, **kwargs):
        return self.tfms.__call__(o, **kwargs)

    def overlapping_splits(self):
        return L(Counter(self.splits.concat()).values()).filter(gt(1))
    ...
    def setup(
        ...
        if len(self) != 0:
            x = (
                ...
                else super().__getitem__(self.splits[0])[0]
            )
            self.types = []
    ...
    def infer_idx(self, x):
        ...
        assert idx < len(
            self.types
        ), f"Expected an input of type in \n{pretty_types}\n but got {type(x)}"
        return idx

    def infer(self, x):
        return compose_tfms(
            x, tfms=self.tfms.fs[self.infer_idx(x) :], split_idx=self.split_idx
        )
...
class Datasets(FilteredBase):
    ...
    def __init__(
        ...
        tfms: MutableSequence
        | Pipeline = None,  # List of `Transform`(s) or `Pipeline` to apply
        ...
        **kwargs
        ...
        self.tls = L(
            tls
            if tls
            else [TfmdLists(items, t, **kwargs) for t in L(ifnone(tfms, [None]))]
        )
    ...
    def __getattr__(self, k):
        return gather_attrs(self, k, "tls")

    def __dir__(self):
        return super().__dir__() + gather_attr_names(self, "tls")
    ...
    def __repr__(self):
        return coll_repr(self)

    def decode(self, o, full=True):
        return tuple(
            tl.decode(o_, full=full) for o_, tl in zip(o, tuplify(self.tls, match=o))
        )
    ...
    def overlapping_splits(self):
        return self.tls[0].overlapping_splits()
    ...
    @property
    def splits(self):
        return self.tls[0].splits

    @property
    def split_idx(self):
        return self.tls[0].tfms.split_idx

    @property
    def items(self):
        return self.tls[0].items
...
def test_set(
    dsets: Datasets
    | TfmdLists,  # Map- or iterable-style dataset from which to load the data
    ...
    if isinstance(dsets, Datasets):
        ...
        raise Exception(
            f"This method requires using the fastai library to assemble your data. Expected a `Datasets` or a `TfmdLists` but got {dsets.__class__.__name__}"
        )
...
def test_dl(
    ...
    **kwargs
    ...
    test_ds = (
        test_set(
            self.valid_ds, test_items, rm_tfms=rm_type_tfms, with_labels=with_labels
        )
        if isinstance(self.valid_ds, (Datasets, TfmdLists))
        else test_items
    )
