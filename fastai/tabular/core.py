...
class Tabular(CollBase, GetAttr, FilteredBase):
    ...
    def __init__(
        ...
        if (
            ...
            and pd.options.mode.chained_assignment is not None
        ):
            warn(
        ...
        self.cat_names, self.cont_names, self.procs = (
            L(cat_names),
            L(cont_names),
            Pipeline(procs),
        )
    ...
    def subset(self, i):
        return self.new(
            self.items[slice(0, self.split) if i == 0 else slice(self.split, len(self))]
        )
    ...
    def decode(self):
        return self.procs.decode(self)

    def decode_row(self, row):
        return self.new(pd.DataFrame(row).T).decode().items.iloc[0]

    def show(self, max_n=10, **kwargs):
        display_df(self.new(self.all_cols[:max_n]).decode().items)
    ...
    def targ(self):
        return self.items[self.y_names]
    ...
    def y(self):
        return self[self.y_names[0]]

    def new_empty(self):
        return self.new(pd.DataFrame({}, columns=self.items.columns))
    ...
    def all_col_names(self):
        ...
        return (
            self.x_names + self.y_names
            if len(ys) == len(self.y_names)
            else self.x_names
        )
...
def _add_prop(cls, nm):
    @property
    def f(o):
        return o[list(getattr(o, nm + "_names"))]
...
class TabularProc(InplaceTransform):
    ...
    def setup(
        self, items=None, train_setup=False
    ):  # TODO: properly deal with train_setup
...
def setups(self, to: Tabular):
    if len(to.y_names) > 0:
        if self.vocab is None:
            self.vocab = CategoryMap(
                getattr(to, "train", to).iloc[:, to.y_names[0]].items, strict=True
            )
        else:
...
def encodes(self, to: Tabular):
    to.transform(
        to.y_names,
        partial(_apply_cats, {n: self.vocab for n in to.y_names}, 0),
        all_col=False,
    )
    return to
...
def decodes(self, to: Tabular):
    to.transform(
        to.y_names,
        partial(_decode_cats, {n: self.vocab for n in to.y_names}),
        all_col=False,
    )
    return to
...
def setups(self, to: Tabular):
    store_attr(
        but="to",
        means=dict(getattr(to, "train", to).conts.mean()),
...
class FillMissing(TabularProc):
    ...
    def setups(self, to):
        ...
        store_attr(
            but="to",
            na_dict={
                n: self.fill_strategy(to[n], self.fill_vals[n])
    ...
    def encodes(self, to):
        ...
        for n in missing.any()[missing.any()].keys():
            assert (
                n in self.na_dict
            ), f"nan values in `{n}` but not in setup training set"
...
class TabDataLoader(TfmdDL):
    ...
    def __init__(
        self, dataset, bs=16, shuffle=False, after_batch=None, num_workers=0, **kwargs
    ):
