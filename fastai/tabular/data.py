...
class TabularDataLoaders(DataLoaders):
    ...
    def from_df(
        ...
        df: pd.DataFrame,
        path: str
        | Path = ".",  # Location of `df`, defaults to current working directory
        ...
        **kwargs
        ...
        splits = (
            RandomSplitter()(df) if valid_idx is None else IndexSplitter(valid_idx)(df)
        )
        to = TabularPandas(
            df, procs, cat_names, cont_names, y_names, splits=splits, y_block=y_block
        )
    ...
    def from_csv(
        ...
        **kwargs
        ...
        return cls.from_df(
            pd.read_csv(csv, skipinitialspace=skipinitialspace), **kwargs
        )
...
TabularDataLoaders.from_csv = delegates(to=TabularDataLoaders.from_df)(
