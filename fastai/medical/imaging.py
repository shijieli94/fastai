...
def get_dicom_files(path, recurse=True, folders=None):
    ...
    return get_files(
        path, extensions=[".dcm", ".dicom"], recurse=recurse, folders=folders
    )
...
class PILDicom(PILBase):
    ...
    def create(cls, fn: Path | str | bytes, mode=None) -> None:
        ...
        if isinstance(fn, bytes):
            im = Image.fromarray(
                pydicom.dcmread(pydicom.filebase.DicomBytesIO(fn)).pixel_array
            )
...
def show(
    self: DcmDataset, scale=True, cmap=plt.cm.bone, min_px=-1100, max_px=None, **kwargs
):
    "Display a normalized dicom image by default"
    px = (
        self.windowed(*scale)
        if isinstance(scale, tuple)
        else self.hist_scaled(min_px=min_px, max_px=max_px, brks=scale)
        if isinstance(scale, (ndarray, Tensor))
        else self.hist_scaled(min_px=min_px, max_px=max_px)
        if scale
        else self.scaled_px
    )
...
def mask_from_blur(x: DcmDataset, window, sigma=0.3, thresh=0.05, remove_max=True):
    "Create a mask from the blurred image"
    return to_device(x.scaled_px).mask_from_blur(
        window, sigma, thresh, remove_max=remove_max
    )
...
def save_tif16(x: Tensor | DcmDataset, path, bins=None, compress=True):
    ...
    Image.fromarray(x.to_uint16(bins)).save(
        str(fn), compression="tiff_deflate" if compress else None
    )
...
class DicomSegmentationDataLoaders(DataLoaders):
    ...
    def from_label_func(
        ...
        **kwargs
        ...
        dblock = DataBlock(
            blocks=(ImageBlock(cls=PILDicom), MaskBlock(codes=codes)),
