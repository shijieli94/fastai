...
def main(
    ...
    sh: Param("Random erase max proportion", float) = 0.0,
    sa: Param("Self-attention", store_true) = False,
    sym: Param("Symmetry for self-attention", int) = 0,
    beta: Param("SAdam softplus beta", float) = 0.0,
    ...
    pr(
        f"pct_noise: {pct_noise}; epochs: {epochs}; lr: {lr}; size: {size}; sqrmom: {sqrmom}; mom: {mom}; eps: {eps}"
    )
    ...
    for run in range(runs):
        ...
        learn = Learner(
            dls,
            m(n_out=10, act_cls=act_fn, sa=sa, sym=sym, pool=pool),
            opt_func=opt_func,
